...
import { QueueName, JobPriority } from '../shared/config';
import type {
  QueueEvent,
  QueueEventType,
  OrderCreatedData,
...
class EventPublisherService implements IEventPublisher {
  ...
  /**
   * Publish generate invoice event
   *
   * @param data - Generate invoice data
   */
  public async publishGenerateInvoice(data: GenerateInvoiceData): Promise<void> {
    await this.publish({
      type: 'GENERATE_INVOICE' as QueueEventType,
      queue: QueueName.ORDERS, // Using ORDERS queue or creates a new INVOICES queue? Plan said GENRATE_INVOICE job. I'll use ORDERS for now as it relates to orders or maybe PAYMENTS. The plan mentioned PaymentService enqueues it. Let's use ORDERS or create a new queue. Using ORDERS queue is safe.
      data,
      timestamp: new Date(),
      priority: JobPriority.NORMAL,
    });
