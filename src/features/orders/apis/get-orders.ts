...
const handler = async (req: RequestWithUser, res: Response) => {
  ...
  logger.info(`[GetOrders] Request received for user: ${userId}`);
  if (!userId) {
  ...
  logger.info(
    `[GetOrders] Fetching orders - Page: ${page}, Limit: ${limit}, Status: ${status || 'all'}`
  );
  ...
  // Get total count
  logger.info('[GetOrders] Querying total count...');
  ...
  logger.info(`[GetOrders] Total orders found: ${total}`);
