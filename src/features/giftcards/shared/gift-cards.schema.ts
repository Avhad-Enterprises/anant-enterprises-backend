 ...
 * Reason: Gift card feature not implemented yet
...
export const giftCards = pgTable(
  'gift_cards',
  {
    ...
    // Ownership & Delivery
    purchaser_user_id: uuid('purchaser_user_id').references(() => users.id, {
      onDelete: 'set null',
    }),
    ...
    source_order_id: uuid('source_order_id').references(() => orders.id, { onDelete: 'set null' }), // CRITICAL FIX #3B
    issued_by_admin_id: uuid('issued_by_admin_id').references(() => users.id, {
      onDelete: 'set null',
    }),
