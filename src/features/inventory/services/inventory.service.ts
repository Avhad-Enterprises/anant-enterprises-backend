...
export async function updateInventory(id: string, data: UpdateInventoryDto, updatedBy: string) {
  ...
  if (data.incoming_po_reference !== undefined)
    updates.incoming_po_reference = data.incoming_po_reference;
 ...
 */
async function resolveLocationForInventory(
  locationId?: string,
  createdBy?: string
): Promise<string> {
...
export async function createInventoryForProduct(
  ...
  let existing;
  if (variantId) {
    // If variant provided, check strictly for that variant's inventory
    existing = await inventoryQueries.findInventoryByVariant(variantId);
  } else {
    // If no variant, check for base product inventory (where variant_id is null usually,
    // but findInventoryByProduct returns all. We should probably filter.
    // For now preventing duplicates is key.)
    existing = await inventoryQueries.findInventoryByProduct(productId);
    // If we are creating base inventory, we want to ensure we don't duplicate logic
    // But let's assume if it exists, we stick to it.
  }

  if (existing.length > 0) {
    // If variantId is specific, existing[0] is correct.
    // If productId is general, and we are creating base, existing might contain variants.
    // However, standard logic:
    logger.info(
      `[Inventory] Product ${productId} ${variantId ? `(Variant ${variantId})` : ''} already has inventory record`
    );
    return existing[0];
  }
