 ...
 */
export type SanitizedUser = Omit<IUser, 'password_hash' | 'reset_token_hash' | 'otp_code'>;

/**
 * Remove sensitive fields from a single user object
 * Ensures password_hash and other sensitive data never leak in API responses
 */
export const sanitizeUser = (user: IUser): SanitizedUser => {
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const { password_hash, reset_token_hash, otp_code, ...sanitized } = user;
  return sanitized;
};
