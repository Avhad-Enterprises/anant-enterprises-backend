...
class CollectionRoute implements Route {
    ...
    private async initializeRoutes() {
        ...
        const { default: addProductsToCollectionRouter } = await import('./apis/add-products-to-collection');
        const { default: removeProductFromCollectionRouter } = await import('./apis/remove-product-from-collection');
        const { default: getAdminCollectionsRouter } = await import('./apis/get-admin-collections');

        // Phase 3: Enhancement Features
        const { default: reorderCollectionProductsRouter } = await import('./apis/reorder-collection-products');
        const { default: deleteCollectionRouter } = await import('./apis/delete-collection');
        const { default: getFeaturedCollectionsRouter } = await import('./apis/get-featured-collections');
...
export { collectionCacheService, CollectionCacheService } from './services/collection-cache.service';
