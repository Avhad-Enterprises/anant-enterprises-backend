  ...
  try {
    ...
    await DB.schema.createTable(CUSTOMIZATION_GROUPS, table => {
      ...
      table
        ...
        .unique()
        .comment(
          'Unique internal reference title for identifying groups & combos (CRM/system use)'
        );
      table.text('group_description').nullable().comment('Detailed description shown under group');
      table
        .boolean('is_required')
        .defaultTo(false)
        .comment('Customer must choose at least one option?');
      table
        .enu('selection_type', ['single', 'multiple'])
        .notNullable()
        .comment('Single select or multi select');
      table.integer('min_required').defaultTo(0).comment('Minimum required selections');
      table.integer('max_allowed').defaultTo(1).comment('Maximum allowed selections');
      table.integer('sort_order').defaultTo(0).comment('For ordering groups');
      table.enu('status', ['active', 'inactive']).defaultTo('active').comment('Activation status');
      table
        .enu('price_visibility', ['show', 'hide'])
        .defaultTo('show')
        .comment('Whether to show price difference');
      table
        .enu('group_type', ['default', 'addon', 'upgrade', 'combo'])
        .defaultTo('default')
