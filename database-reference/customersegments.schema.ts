...
export const seed = async (dropFirst = false) => {
  try {
    ...
    await DB.schema.createTable(CUSTOMER_SEGMENTS, table => {
      ...
      table.string("segment_code", 100)
        .notNullable()
        .unique()
        .comment("Short internal code (e.g., VIP30D, HIGHSPENDER)");
      table.string("name", 255)
        .notNullable()
        .comment("User-visible segment name");
      ...
      table.string("auto_refresh_interval", 50)
        .nullable()
        .comment("Frequency: 15m, 1h, 6h, daily");
      table.integer("estimated_users_count")
        .defaultTo(0)
