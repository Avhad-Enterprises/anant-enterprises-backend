...
export const seed = async (dropFirst = false) => {
    try {
        ...
        await DB.schema.createTable(GIFT_CARD, table => {
            ...
            table.string('code', 100).notNullable().unique().comment('The actual redeemable code (e.g., GIFT-3AB9KQ)');
            ...
            table.enu('status', [
                'active', 'partially_used', 'redeemed', 'expired', 'disabled'
            ]).defaultTo('active');

            table.bigInteger('user_id').nullable().comment('FK to users (nullable for guest checkout)');

            table.bigInteger('issued_by').unsigned().nullable().comment('Admin ID who created (if manual)');
