...
export const seed = async (dropFirst = false) => {
  try {
    ...
    await DB.schema.createTable(PRODUCT_TABLE, table => {
      ...
      table.decimal('cost_price', 10, 2).defaultTo(0.0).comment('Cost');
      table.decimal('selling_price', 10, 2).defaultTo(0.0).comment('Selling price');
      ...
      table
        .enum('sales_channel', ['website', 'app', 'others'])
        .defaultTo('website')
