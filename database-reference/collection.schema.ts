...
export const seed = async (dropFirst = false) => {
  try {
    ...
    await DB.schema.createTable(COLLECTIONS, (table) => {
      ...
      table.string('title', 255).notNullable().comment('Collection name (e.g. “Summer Essentials”)');
      table.string('handle', 255).unique().notNullable().comment('URL-friendly slug (like /collections/summer-essentials)');
      table.text('description').nullable().comment('Description (rich text or plain)');

      table.enu('type', ['manual', 'automated'], {
        useNative: true,
        enumName: 'collection_type'
      }).defaultTo('manual').comment('Type of collection');

      table.jsonb('rules_json').nullable().comment('Used when type = automated');

      table.enu('rule_match_type', ['all', 'any'], {
        useNative: true,
        enumName: 'rule_match_type_enum'
      }).defaultTo('all').comment('Whether all or any rules must match');

      table.enu('sort_order', [
        'manual',
        'title-asc',
        'title-desc',
        'price-asc',
      ...
      }).defaultTo('manual').comment('Sorting preference');
      ...
      table.string('mobile_image', 255)
        .nullable()
        .comment('S3 key for mobile image');
