...
export const seed = async (dropFirst = false) => {
  try {
    ...
    await DB.schema.createTable(GIFTCARD_transaction, table => {
      ...
      table.bigInteger('order_id').unsigned().nullable().comment('Order ID if redeemed');

      table.decimal('amount', 12, 2).notNullable().comment('Amount redeemed/adjusted');

      table
        .enu('type', ['issue', 'redeem', 'refund', 'adjustment'], {
          useNative: true,
      ...
      table.bigInteger('performed_by').unsigned().nullable().comment('Admin/User who made change');

      table.string('notes', 255).nullable().comment('Optional notes');
