...
export const seed = async (dropFirst = false) => {
    try {
        ...
        await DB.schema.createTable(DISCOUNTS, table => {
            ...
            table.enum("create_type", ["single", "bulk"])
                .defaultTo("single")
            ...
            table.enum("applies_to", ["entire_order", "specific_products", "specific_collections"])
                .notNullable()
            ...
            table.enum("min_requirement_type", ["none", "min_purchase_amount", "min_quantity", "min_items"])
                .defaultTo("none")
                .comment("Condition type");

            table.decimal("min_requirement_value", 12, 2).nullable()
            ...
            table.enum("status", ["draft", "active", "scheduled", "expired", "paused"])
                .defaultTo("draft")
                .comment("Lifecycle state");
            table.boolean("combines_with_product").defaultTo(false).comment("Can combine with product discounts");
            table.boolean("combines_with_order").defaultTo(false).comment("Can combine with order discounts");
            table.boolean("combines_with_shipping").defaultTo(false).comment("Can combine with shipping discounts");
            table.specificType("prereq_customer_groups_json", "jsonb").nullable().comment("Customer groups allowed");
            table.specificType("prereq_customer_tags_json", "jsonb").nullable().comment("Customer tags required");
            table.specificType("prereq_shipping_countries_json", "jsonb").nullable().comment("Allowed shipping countries");
            table.specificType("rule_products_json", "jsonb").nullable().comment("Applicable products list");
            table.specificType("rule_collections_json", "jsonb").nullable().comment("Applicable collections list");
            table.specificType("bxgy_rules_json", "jsonb").nullable().comment("Buy X Get Y structured offer rules");
            table.specificType('tags', 'jsonb').nullable().defaultTo(DB.raw(`'[]'::jsonb`)).comment('Tags or labels');
            table.specificType('notes', 'jsonb').nullable().defaultTo(DB.raw(`'[]'::jsonb`)).comment('Internal notes');
