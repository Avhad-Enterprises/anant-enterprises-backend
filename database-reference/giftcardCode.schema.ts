    ...
    try {
        ...
        await DB.schema.createTable(GIFTCARDCODE, (table) => {
            ...
            table.string('prefix', 50)
                .nullable()
                .comment('Optional code prefix (e.g., GBC-, NEWYEAR-)');

            table.string('suffix', 50)
                .nullable()
                .comment('Optional code suffix (e.g., -2025)');

            table.integer('length')
                .defaultTo(8)
                .notNullable()
                .comment('Length of the random portion');

            table.enu('character_set', ['alphabets', 'numbers', 'alphanumeric'], {
                useNative: true,
            ...
            table.boolean('include_uppercase')
                .defaultTo(true)
                .notNullable()
                .comment('Include uppercase letters (A–Z)');

            table.boolean('include_lowercase')
                .defaultTo(false)
                .notNullable()
                .comment('Include lowercase letters (a–z)');

            table.boolean('include_numbers')
                .defaultTo(true)
                .notNullable()
                .comment('Include numbers (0–9)');

            table.string('separator', 1)
                .nullable()
                .comment('e.g., - for grouping codes like ABCD-1234');

            table.integer('segment_length')
                .defaultTo(0)
                .notNullable()
