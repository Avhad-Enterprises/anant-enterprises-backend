...
async function seedUsersAndReviews() {
    ...
    const existingProducts = await db.select({ id: products.id, sku: products.sku }).from(products).limit(5);
    ...
    if (allTestUserIds.length > 0) {
        ...
        await db.delete(businessCustomerProfiles).where(inArray(businessCustomerProfiles.user_id, allTestUserIds));
    ...
    const techHQAddress = insertedAddresses.find(a => a.user_id === techSolutions.id && a.address_type === 'company');
    const techShipAddress = insertedAddresses.find(a => a.user_id === techSolutions.id && a.address_type === 'shipping');
    ...
    const billingAddressRahul = insertedAddresses.find(a => a.user_id === rahul.id && a.address_type === 'billing');
    ...
    const questionData = [
        {
            ...
            answer: 'Yes, the AquaPure RO 500 comes with a built-in booster pump that works efficiently even with low water pressure (as low as 0.3 kg/cm²).',
        ...
        },
        {
            ...
            answer: 'The AquaPure RO 500 comes with a comprehensive 5-year warranty on the purifier body and 1-year warranty on electrical components.',
            answered_by: adminUser.id,
            status: 'answered' as const,
            is_public: true,
        },
        ...
        },
        {
            ...
            answer: 'Yes! The UV lamp can be easily replaced at home. Simply open the top panel and slide out the old lamp. No tools required. We recommend replacement every 8000 hours.',
            answered_by: adminUser.id,
            status: 'answered' as const,
            is_public: true,
        },
        {
            ...
            answer: 'No, UV purifiers do not reduce TDS. They are designed to kill bacteria and viruses. For TDS reduction, you need an RO purifier.',
            answered_by: adminUser.id,
            status: 'answered' as const,
            is_public: true,
        },
        {
            ...
            answer: 'Yes, we offer Annual Maintenance Contracts for all commercial products. Contact our B2B team at business@anantenterprises.com for customized plans.',
            answered_by: adminUser.id,
            status: 'answered' as const,
            is_public: true,
        },
        {
            ...
            answer: 'For optimal performance, we recommend replacing the sediment filter cartridge every 3-6 months depending on your water quality and usage.',
            answered_by: adminUser.id,
            status: 'answered' as const,
            is_public: true,
        },
